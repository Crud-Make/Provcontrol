<div class="bg-slate-800/80 border rounded-xl p-6 border-l-4"
     :class="Math.abs(resultado.diferenca) < 0.01 ? 'border-l-emerald-500 border-slate-700' : 'border-l-brandred-500 border-slate-700'"
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div class="bg-slate-900/50 rounded-lg p-4">
            ...
            <p class="text-xl font-bold text-brand-300" x-text="formatMoney(resultado.total_concentrador)"></p>
        </div>
        <div class="bg-slate-900/50 rounded-lg p-4">
        ...
        <div class="bg-slate-900/50 rounded-lg p-4">
            ...
            <p class="text-xl font-bold"
               :class="Math.abs(resultado.diferenca) < 0.01 ? 'text-emerald-400' : 'text-brandred-300'"
            ...
            <p x-show="resultado.diferenca < -0.01" class="text-sm text-brandred-300">Falta no caixa</p>
        </div>
        <div class="bg-slate-900/50 rounded-lg p-4">
            <p class="text-sm text-slate-400">Taxas (cartões/apps)</p>
            <p class="text-xl font-bold text-brandred-300" x-text="formatMoney(resultado.total_taxas)"></p>
        </div>
    </div>
</div>
